...
class photoresister (threading.Thread):
	def __init__ (self):
		threading.Thread.__init__(self)
	...
	def run(self):
		...
		status = 1
		light = 'Light : '+str(ADC.read(0))
        	while True:
                	print  json.dumps({"d" : {'Light' : str(ADC.read(0))}, "Source" : addr , "TimeStamp" : time.time(), "CommunicationTech" : "XBEE"})
                	time.sleep(5)
...
#setting photoresister sensor
thread =photoresister()
thread.start()
threads.append(thread)
